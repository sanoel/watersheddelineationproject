...
public class MergedPit {
	...
	double minOutsidePerimeterElevation;
	...
	public MergedPit(FilledDataset fillDataset) {
			...
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++){
					...
					if (pits[r+y][c+x] != pits[r][c]) {
						...
						onBorder = true;
						if (Double.isNaN(overflowPointElevation) || (currentElevation <= overflowPointElevation && neighborElevation <= overflowPointElevation)) {
							minOutsidePerimeterElevation = neighborElevation;
							minInsidePerimeterElevation = currentElevation;
							overflowPointElevation = Math.max(neighborElevation, currentElevation);
