...
//	List<Point> allPointsList; // this is used to speed up several calculations when pits merge: Re-IDing the two merging pits, and finding new retention volume critical to causal sequential depression filling
	...
	Point pitOutletPoint; // point at which the spilloverElevation occurs (drains toward pit)
	Point outletSpilloverFlowDirection;  // cell to which this pit will overflow (is not inside this current pit)
	int area;
...
//	}
	...
	public void completePitConstruction(List<Point> indicesDrainingToPit, float[][] drainage, float cellSize, float[][] dem, int[][] pitIdMatrix) {		
		...
		for (int i = indicesDrainingToPit.size()-1; i > -1; i--) {
...
//			for (int x = -1; x < 2; x++) {
		...
		filledVolume = 0.0f;
		pitDrainageRate = 0.0f;
		float netAccumulationRate = (DelineationAppConfigs.rainfallIntensity * indicesDrainingToPit.size() * cellSize*cellSize);//cubic meters per hour          - pitDrainageRate
		spilloverTime = retentionVolume / netAccumulationRate; //hours
	}
...
//			for (int x = -1; x < 2; x++) {
		...
		//Now find retentionVolume
//		List<Point> indicesToCheck = new ArrayList<Point>(area);
//		indicesToCheck.add(pitPoint);
...
//			if (dem[r][c] <= spilloverElevation) {
//				dem[r][c] = spilloverElevation;
//		    //cells that aren't raised may be updated with the new ID
//			} else {
//				pitIdMatrix[r][c] = pitId;;
//			}
//			if (dem[r][c] < spilloverElevation) {
//				retentionVolume += ((spilloverElevation - dem[r][c]) * cellSize * cellSize);
//			}
...
//			   " \nArea: " + Integer.toString(area) +
			   ...
			   " \nRainfall Intensity: " + Float.toString(DelineationAppConfigs.rainfallIntensity);
	}
